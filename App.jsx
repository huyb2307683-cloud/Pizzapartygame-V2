...
const App = () => {
  ...
  // TRẠNG THÁI ĐIỂM VÀ LỖI
  const [stats, setStats] = useState({ correct: 0, wrong: 0 });
  const [wrongCount, setWrongCount] = useState(0); // Đếm số lần sai trong 1 màn
  ...
  // Hàm chuyển màn chơi dùng chung
  ...
  // QUAN TRỌNG: KIỂM TRA ĐÁP ÁN VỚI QUY TẮC 3 LẦN SAI
  const checkNumericalAnswer = () => {
    ...
    if (isCorrect) {
      setFeedback('correct');
      // Thưởng điểm nếu đúng ngay lần đầu
      ...
      if (newWrongCount >= 3) {
        setTimeout(() => {
          alert("Bạn đã trả lời sai 3 lần. Chúng ta hãy cùng sang thử thách tiếp theo nhé!");
          setStats(prev => ({ ...prev, wrong: prev.wrong + 1 }));
          handleNextLevel();
        }, 1200);
      } else {
  ...
  if (isLoading) return <div className="flex h-screen items-center justify-center text-5xl agbalumo animate-pulse">🍕 Đang chuẩn bị bài học...</div>;
        ...
        <div className="bg-white rounded-[60px] shadow-2xl border-8 border-rose-400 p-12 max-w-2xl w-full text-center agbalumo">
          ...
          <div className="grid grid-cols-2 gap-8 mb-12">
            <div className="bg-green-50 p-8 rounded-[40px] border-4 border-green-200">
              <p className="text-2xl text-green-700 mb-2 font-bold">CÂU ĐÚNG</p>
              <p className="text-7xl text-green-600">{stats.correct}</p>
            </div>
            <div className="bg-red-50 p-8 rounded-[40px] border-4 border-red-200">
              <p className="text-2xl text-red-700 mb-2 font-bold">CÂU SAI/BỎ QUA</p>
  ...
  return (
    <div ref={containerRef} className="game-container agbalumo select-none relative h-screen w-screen overflow-hidden" style={{ touchAction: 'none' }} onPointerMove={handlePointerMove} onPointerUp={handlePointerUp}>
      <div className="bg-white/95 p-6 border-b-4 border-rose-300 flex flex-col items-center justify-center text-center z-20 shadow-md">
        <div className="text-rose-500 text-xl mb-1">Màn {currentLevelIdx + 1}/10 • Lần thử: {wrongCount}/3</div>
        <p className="text-2xl md:text-3xl lg:text-4xl text-rose-900 font-bold">{currentLevel?.problem}</p>
      </div>
